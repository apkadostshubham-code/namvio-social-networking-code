...
<li data-nav="${esc(item.id)}" class="nv-nav-enter" style="animation-delay:${i * 40}ms">
  <a href="#" onclick="${routeOnclick(item.id, false)}">
    ...
    function renderSidebarItem(item, index) {
        ...
        const adminClass = item.adminOnly ? ' d-none' : '';
        const delay = (index || 0) * 32;
        return `
<li id="sidemenu-${esc(item.id)}" data-nav="${esc(item.id)}" class="nv-side-nav-item nv-nav-enter${adminClass}"${adminAttr} style="animation-delay:${delay}ms">
  <a href="#" onclick="${routeOnclick(item.id, false)}">
    <span class="nv-side-icon-wrap"><i class="fa-solid ${esc(item.icon)}"></i></span>
    <span class="nv-side-label">${esc(item.label)}</span>
    ${badgeHtml(item)}
  </a>
</li>`;
    }

    function renderSidebarSection(section, isFirst, startIndex) {
        let idx = startIndex || 0;
        const items = section.items.map((item) => renderSidebarItem(item, idx++)).join('');
        const labelClass = isFirst ? 'nv-side-nav-label' : 'nv-side-nav-label mt-3';
        return {
            html: `
  <div class="nv-side-nav-group">
...
<a href="#" class="mobile-nav-item nv-bottom-nav-item nv-nav-enter" data-mobile-nav="${esc(item.id)}"
  ...
  <span>${esc(label)}</span>
    ...
    function renderSidebar() {
        ...
        if (!host) return;
        let itemIndex = 0;
        const sections = SIDEBAR_SECTIONS.map((section, i) => {
            const block = renderSidebarSection(section, i === 0, itemIndex);
            itemIndex = block.nextIndex;
            return block.html;
        }).join('');
        host.innerHTML = `
<div class="nv-side-nav-inner position-relative">
  <div class="nv-side-nav-indicator" id="nv-side-nav-indicator" aria-hidden="true"></div>
  ${sections}
</div>`;
    }
...
<ul class="list-unstyled mb-0 nv-mobile-nav-list nv-mobile-nav-secondary">${MOBILE_EXTRA.map((item, i) => renderMobileItem(item, true, i + PRIMARY.length)).join('')}</ul>`;
    ...
    function positionSidebarIndicator() {
        const track = document.querySelector('.nv-side-nav-inner');
        const indicator = document.getElementById('nv-side-nav-indicator');
        const active = track ? track.querySelector('.side-menu-list li.active') : null;
        if (!track || !indicator || !active) {
            if (indicator) indicator.style.opacity = '0';
            return;
        }
        const tr = track.getBoundingClientRect();
        const ar = active.getBoundingClientRect();
        indicator.style.top = ar.top - tr.top + track.scrollTop + 'px';
    ...
    function setActiveClasses(viewId) {
        ...
        document.querySelectorAll('.side-menu-list li[data-nav]').forEach((li) => {
            li.classList.toggle('active', li.dataset.nav === activeView);
        });
    }
