...
(function () {
    ...
    const REVEAL_SELECTOR =
        '.card-component, .nv-feed-hero, .nv-feed-composer, .nv-feed-quick-chip, .nv-sponsor-block, .nv-sponsor-feed-wrap, .network-member-card, .listing-card, .hof-hero, .hof-kpi-card, .hof-podium-slot, .hof-podium-shell, .hof-your-rank, .hof-toolbar-shell, .hof-board-shell, .hof-quick-chip, .mp-page-hero, .mp-kpi-card, .mp-listing-card, .mp-toolbar-shell, .mp-quick-chip, .net-page-hero, .net-kpi-card, .net-member-card, .net-invite-shell, .net-toolbar-shell, .net-events-shell, .net-quick-chip, .nv-sidebar-profile, .nv-side-nav-shell, .nv-side-nav-item, .nv-mobile-nav-hero, .profile-hero-card, .profile-section-shell, .profile-quick-chip, .profile-tabs-card, .profile-stat-cell, .profile-metric-pill, .lg-page-hero, .lg-kpi-card, .lg-nav-shell, .lg-accordion-shell, .lg-policy-item, .mkt-page-hero, .mkt-kpi-card, .mkt-section-card, .mkt-upgrade-shell, .mkt-follow-shell, .mkt-quick-chip, .msg-page-hero, .msg-thread-item, .sp-page-hero, .sp-kpi-card, .sp-wall-card, .nv-doc-hero, .nv-guide-section, .nv-rules-section, .nv-help-hub-card';
    ...
    function staggerChildren(container, itemSelector) {
        if (!container || reducedMotion || typeof container.querySelectorAll !== 'function') return;
    ...
    function decorateInteractive() {
        document.querySelectorAll('#view-feed .btn-primary, .nv-feed-composer .btn-primary').forEach((btn) => {
            if (!btn.classList.contains('nv-btn-shine')) btn.classList.add('nv-btn-shine');
        });
        document.querySelectorAll('.nv-feed-quick-chip, .mp-quick-chip, .mkt-quick-chip').forEach((chip, i) => {
            if (!reducedMotion) chip.style.animationDelay = i * 60 + 'ms';
        });
        document.querySelectorAll('.mp-cta-btn, .mkt-upgrade-shell .btn-primary').forEach((btn) => {
            if (!btn.classList.contains('nv-btn-shine')) btn.classList.add('nv-btn-shine');
        });
    }
