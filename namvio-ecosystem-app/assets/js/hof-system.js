...
(function () {
    ...
    function getTier(score) {
        if (window.NamvioReputation && window.NamvioReputation.getTier) {
            return window.NamvioReputation.getTier(score);
        }
...
<tr class="hof-leader-row${topClass}" data-hof-id="${esc(m.id)}">
  ...
  <td class="hof-col-rep text-right">
    ...
    ${window.NamvioReputation ? window.NamvioReputation.renderCompact(m.score) : ''}
  </td>
</tr>`;
    }
...
<p class="hof-table-foot text-muted mb-0">Showing ${rows.length} of ${HOF_MEMBERS.length} Hall of Fame members</p>`;
    ...
    function refreshBoard() {
        ...
        if (window.NamvioMotion) {
            const view = document.getElementById('view-hof');
            window.NamvioMotion.scanReveal(view);
            if (view) window.NamvioMotion.staggerChildren(view, '.hof-leader-row, .hof-podium-slot');
        }
    }
    ...
    function init(session) {
        ...
        if (window.NamvioMotion) {
            const view = document.getElementById('view-hof');
            window.NamvioMotion.scanReveal(view);
            if (view) window.NamvioMotion.staggerChildren(view, '.hof-leader-row, .hof-podium-slot, .hof-kpi-card');
        }
    }
