...
const NamvioProfile = (function () {
    ...
    function buildReputationHistoryFromLogs(session) {
        ...
        const logs =
            window.NamvioReputation && window.NamvioReputation.getLogsForUser
                ? window.NamvioReputation.getLogsForUser(handle, 200)
                : [];
    ...
    function renderHeroMetrics(session) {
        ...
        const tier =
            window.NamvioReputation && window.NamvioReputation.getTier
                ? window.NamvioReputation.getTier(rep).name
                : 'Member';
        return `
    ...
    function render(session, opts) {
        ...
        if (window.NamvioReputation && document.getElementById('identity-reputation-block')) {
            setHtml(
                ...
                window.NamvioReputation.renderProfileBlock(session.reputationScore)
            );
        }
        ...
        requestAnimationFrame(() => {
            initLazySections(session);
            if (window.NamvioMotion) {
                const view = document.getElementById('view-identity');
                window.NamvioMotion.scanReveal(view);
                if (view) {
                    window.NamvioMotion.staggerChildren(
                        view,
                        '.profile-stat-cell, .profile-metric-pill, .profile-section-shell'
                    );
                }
            }
        });
    ...
    function shareProfile() {
        if (window.NamvioShare) {
            window.NamvioShare.openProfile();
            return;
        }
        const session = window.activeSessionState;
    ...
    function followUser(btn) {
        ...
        if (!following) {
            if (window.NamvioModeration && window.activeSessionState) {
                window.NamvioModeration.recordMassFollow(window.activeSessionState);
            }
            if (window.nvApplyRep) window.nvApplyRep('GAIN_FOLLOWER', { note: 'Profile follow' });
            const session = window.activeSessionState;
    ...
    function reportUser() {
        ...
        if (window.NamvioReportUser) {
            window.NamvioReportUser(handle);
            return;
        }
