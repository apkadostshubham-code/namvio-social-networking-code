...
<div class="nv-sponsor-feed-wrap mb-3" data-sponsored-feed="${esc(sponsor.id)}">
  ...
  ${renderSponsorCard(sponsor, { placement: 'feed' })}
    ...
    function injectFeedSponsors() {
        ...
        if (container.querySelector('[data-sponsored-feed]')) return;
        if (typeof container.insertAdjacentHTML !== 'function') return;

        const childList = container.children;
        const children = childList ? Array.from(childList) : [];
        if (!children.length) {
        ...
        const liveChildren = container.children ? Array.from(container.children) : [];
        if (liveChildren.length > 5) {
            const second = liveChildren[Math.min(6, liveChildren.length - 1)];
            if (second && !second.dataset?.sponsoredFeed && typeof second.insertAdjacentHTML === 'function') {
