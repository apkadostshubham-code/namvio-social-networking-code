...
function initFeed() {
    ...
    if (window.NamvioFeedHome) window.NamvioFeedHome.refreshStats();
    if (window.NamvioMotion) window.NamvioMotion.staggerFeed();
}
...
function initMarketplace() {
    if (window.NamvioListings) window.NamvioListings.init();
}
...
function filterNetworkingMembers(query) {
    if (window.NamvioNetworking && typeof window.NamvioNetworking.filter === 'function') {
        window.NamvioNetworking.filter(query);
        return;
    }
    const grid = document.getElementById('networking-members-grid');
    if (!grid) return;
...
function rsvpNetworkingEvent(btn) {
    if (!btn) return;
    if (btn.dataset.rsvped === '1') {
        btn.textContent = 'RSVP';
        btn.classList.remove('btn-success');
        btn.classList.add('btn-outline-primary');
        btn.dataset.rsvped = '0';
        return;
    }
    btn.textContent = 'Going';
    btn.classList.remove('btn-outline-primary');
    btn.classList.add('btn-success');
    btn.dataset.rsvped = '1';
}

function connectNetworkingMember(btn, memberId) {
    const member = NETWORKING_MEMBERS.find((m) => m.id === memberId);
    if (!btn || !member) return;
    if (!activeSessionState.networking) {
        activeSessionState.networking = { connections: 128, pending: 6 };
    }
    const net = activeSessionState.networking;
    if (btn.dataset.connected === '1') {
        btn.innerHTML = '<i class="fa-solid fa-user-plus mr-1"></i> Connect';
        btn.classList.remove('btn-success');
        btn.classList.add('btn-primary', 'net-connect-btn');
        btn.dataset.connected = '0';
        net.pending = (net.pending || 0) + 1;
        net.connections = Math.max(0, (net.connections || 128) - 1);
        bumpProfileStat('followers', -1);
        saveSession();
        syncIdentityUI();
        if (window.NamvioNetworking) window.NamvioNetworking.syncStats(net);
        return;
    }
    btn.textContent = 'Connected';
    btn.classList.remove('btn-primary', 'net-connect-btn');
    btn.classList.add('btn-success');
    btn.dataset.connected = '1';
    if (window.NamvioModeration) window.NamvioModeration.recordMassFollow(activeSessionState);
    nvApplyRep('GAIN_FOLLOWER', { note: 'Followed ' + memberId });
    bumpProfileStat('followers', 1);
    net.pending = Math.max(0, (net.pending || 6) - 1);
    net.connections = (net.connections || 128) + 1;
    saveSession();
    syncIdentityUI();
    if (window.NamvioNetworking) window.NamvioNetworking.syncStats(net);
}
...
function initHallOfFame() {
    if (window.NamvioHof && document.getElementById('hof-leaderboard')) {
        window.NamvioHof.init(activeSessionState);
        return;
    }
...
function openMesageThread(peerId) {
    ...
    if (window.NamvioMesages) {
        window.NamvioMesages.onThreadOpen(peerId);
    }
    saveSession();
    updateMesageBadges();
}
...
function mesageBackToList() {
    ...
    if (window.NamvioMesages) {
        window.NamvioMesages.onThreadClose();
    }
    saveSession();
}
...
function filterMesageThreads(query) {
    if (window.NamvioMesages) {
        window.NamvioMesages.syncChrome(query != null ? query : '');
    }
}
...
function sendMesage(e) {
    ...
    saveSession();
    if (window.NamvioMesages) {
        window.NamvioMesages.onMessageSent(peerId);
    }
    ...
    setTimeout(() => {
        ...
        saveSession();
        if (window.NamvioMesages) {
            window.NamvioMesages.onAutoReplyRendered(peerId);
        }
    }, 900);
}
...
function initMarketPulse() {
    ...
    if (window.NamvioMarketPulse) window.NamvioMarketPulse.init();
    else if (window.NamvioMotion) window.NamvioMotion.scanReveal(document.getElementById('view-market-pulse'));
}
...
function syncSidebarProfile() {
    ...
    if (badgeEl) {
        const role = activeSessionState.role || 'elite';
        const roleClass = ROLE_CLASS[role] || 'badge-elite';
        const roleLabel = ROLE_LABEL[role] || 'Member';
        const icon = role === 'elite' ? 'fa-gem' : role === 'broker' ? 'fa-handshake' : role === 'registrar' ? 'fa-server' : 'fa-chart-line';
        badgeEl.className = 'identity-badge ' + roleClass;
        badgeEl.innerHTML = '<i class="fa-solid ' + icon + ' mr-1"></i> ' + escapeHtml(roleLabel);
    }
    if (window.NamvioFeedHome) window.NamvioFeedHome.refreshStats();
}
...
function routeView(targetViewToken, clickedTriggerElement) {
    ...
    if (viewId === 'legal' && window.NamvioDocs) window.NamvioDocs.initLegal();
    if (viewId === 'guide' && window.NamvioDocs) window.NamvioDocs.initGuide();
    if (viewId === 'rules' && window.NamvioDocs) window.NamvioDocs.initRules();
    if (viewId === 'help' && window.NamvioDocs) window.NamvioDocs.initHelp();
    if (viewId === 'support' && window.NamvioSupport) window.NamvioSupport.init();
    if (window.NamvioMotion) window.NamvioMotion.onRouteChange(viewId);
    if (viewId === 'admin' && window.NamvioModeration) window.NamvioModeration.initDashboard(activeSessionState);
    if (viewId === 'identity') {
        syncIdentityUI();
        if (window.NamvioProfile) window.NamvioProfile.render(activeSessionState, { isSelf: true, force: true });
    }
}
...
function bootNamvio() {
    ...
    window.rsvpNetworkingEvent = rsvpNetworkingEvent;
    ...
    window.focusFeedComposer = focusFeedComposer;
    ...
    window.TRENDING_KEYWORDS = TRENDING_KEYWORDS;
    ...
    try {
        ...
        ensureSessionPersisted();
        initFeed();
        if (window.NamvioFeedHome) window.NamvioFeedHome.init();
        initHallOfFame();
        initWhoToFollow();
        initBlogs();
        syncInviteCodeUI();
